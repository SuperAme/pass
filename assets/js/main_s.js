  ...
  $(".qa-accordion").collapse({
    ...
    */
    ...
    function addUsers() {
        ...
        var preciosSae = [[10225,3070,5607,12768,22533,38140],[6620,1986,3364,7660,13520,22884],[517,650,780,894,1006,1116,1228,1342,1452,1563,1615,1669,1721,1773,1825,1842,1929,1980,2033,2085,107]];
        //var preciosNoi = [[5550,1668,3112,7080,11227],[3330,1000,1867,4248,6736],[309,394,473,589,649,707,766,823,883,943,94]];
        var preciosNoi = [[5883,1768,3299,7505,11901],[3530,1060,1979,4503,7140],[328,418,501,624,688,749,812,872,936,1000,100]];
        var preciosCoi = [[5507,1656,3085,7027,11013],[3305,995,1850,4215,6608],[317,401,481,589,661,722,781,841,902,962,95]];
        var preciosBanco = [[4850,1454,2714,6185,9692],[2910,872,1628,3710,5815],[245,307,367,460,507,553,597,644,689,735,73]];
        var preciosProd = [[7105,2132,3980,9050,14275],[4263,1280,2388,5430,8565],[339,425,508,636,700,765,826,890,952,1018,102]];
        var preciosCaja = [[2890,2310],[1734,1386],[210,445,648,849,1050,1250,1451,1652,1853,2054,210]];
        var preciosFacture = [[1575],[945],[88]];
        var precios = [];
        var defaultUsers = [];
        var precioFinal = 0;
        var precioTotal = 0;
        ...
        switch($("#precio-tipo").text()) {
            case "Compra":
                ...
                $("#btn-comprar").attr("data_price", precioTotal);
            ...
            case "Renta":
                ...
                //console.log(precioFinal);
                $("#btn-comprar").attr("data_price", precioFinal);
                precioFinal = parseInt(precioFinal).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,");
                $("#price-add-user").text("$"+precioFinal);
    ...
    $(".num-timbres a").click(function(){
        ...
        var preciosTimbrado = [[50,200,400,1000,2000,5000,10000,20000,50000,70000,100000],[190,380,480,900,1360,2850,5000,9000,21000,27300,37000]];
        var precioFinal = 0;
    ...
    */
    ...
    $(".contact-form-aream #submit").click(function(){
        ...
        if (!isCorrect) {
            ...
            $.post("../php/mail.php", $("#contact-form").serialize())
                .done(function(data) {
                if(data) {
                    $("#modal-contact").modal();
                }
